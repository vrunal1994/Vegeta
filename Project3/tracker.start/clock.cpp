...
Clock::Clock() :
  ...
  currTicks(0), prevTicks(0), ticks(0), fps(0), avgFPS(0)
  {
  start();
}
...
void Clock::display() const { 
  ...
  IOManager::getInstance()
    .printMessageValueAt("FPS ", lastFrames, 10, 70);
...
static int sum=1;
std::deque< unsigned int> store(200,0);
  ...
  counter++;
   store.push_back(lastFrames);
 }
if(frames >200)
{sum += store.back();				//Using deque to have continous influx and outflux of frames for 200frames
 }
 fps = totalFPS/counter;
 avgFPS =sum/counter;
//IOManager::getInstance()
 //   .printMessageValueAt("Overall avg : ",totalFPS/counter , 10, 140);		// Average of All the frames
 
  //IOManager::getInstance()
   // .printMessageValueAt("avg for last 200: ",sum/counter, 10, 160);		// Average of last 200 frames
