...
ScaledSprite::ScaledSprite(const std::string& name, 
    ...
    Drawable(name,
           ...
           Vector2f((rand()%2?1:2)*Gamedata::getInstance().getRandFloat(
            Gamedata::getInstance().getXmlInt(name+"/cushion"),
            Gamedata::getInstance().getXmlInt(name+"/speedX") ),
          (rand()%2?1:2)*Gamedata::getInstance().getRandFloat(
            Gamedata::getInstance().getXmlInt(name+"/cushion"),
            Gamedata::getInstance().getXmlInt(name+"/speedY"))
        )
  ), 
  scale(Gamedata::getInstance().getRandInRange(Gamedata::getInstance().getXmlFloat(name+"/scale/min"),
