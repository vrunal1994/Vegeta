...
ExplodingAI::ExplodingAI(const Sprite* s) :
  Sprite(*s), 
  chunks(), 
  freeList(),
  frames() {
...
ExplodingAI::~ExplodingAI() { 
  for ( unsigned int i = 0; i < frames.size(); ++i ) {
    delete frames[i]; // ExplodingAI made them, so it deletes them
  }
  chunks.clear();     // clearing a vector is a waste of time, but ...
  freeList.clear();   // still ...
}

void ExplodingAI::draw() const { 
...
void ExplodingAI::update(Uint32 ticks) { 
...
void ExplodingAI::makeChunks(unsigned int n) { 
