...
ExplodingPlayer::ExplodingPlayer(const Sprite& s) :
  Sprite(s), 
  chunks(), 
  freeList(),
  frames(),
...
ExplodingPlayer::~ExplodingPlayer() { 
  for ( unsigned int i = 0; i < frames.size(); ++i ) {
    delete frames[i]; // ExplodingPlayer made them, so it deletes them
  }
  chunks.clear();     // clearing a vector is a waste of time, but ...
  freeList.clear();   // still ...
}

void ExplodingPlayer::draw() const { 
...
void ExplodingPlayer::update(Uint32 ticks) { 
  ...
  //if(chunks.size() ==0) { isSpriteDone = true;}
  
}

void ExplodingPlayer::makeChunks(unsigned int n) { 
  // Break the SDL_Surface into n*n squares; where each square
  // has width and height of frameWidth/n and frameHeight/n
  // std::cout<<"Making chunks"<<std::endl;
