#include <iostream>
#include <cmath>
#include "explodingVegeta.h"

ExplodingVegeta::ExplodingVegeta(const Vegeta& s) :
  Vegeta(s), 
  chunks(), 
  freeList(),
  frames() {
...
ExplodingVegeta::~ExplodingVegeta() { 
  for ( unsigned int i = 0; i < frames.size(); ++i ) {
    delete frames[i]; // ExplodingVegeta made them, so it deletes them
  }
  chunks.clear();     // clearing a vector is a waste of time, but ...
  freeList.clear();   // still ...
}

void ExplodingVegeta::draw() const { 
  // Override draw; use the draw in Chunk, which derives from Vegeta.
  // So the draw we're using is actually in Vegeta
  std::list<ChunkVegeta>::const_iterator ptr = chunks.begin();
...
void ExplodingVegeta::update(Uint32 ticks) { 
  std::list<ChunkVegeta>::iterator ptr = chunks.begin();
  while (ptr != chunks.end()) {
    ptr->update(ticks);
...
void ExplodingVegeta::makeChunks(unsigned int n) { 
  ...
  SDL_Surface* VegetaSurface(getFrame()->getSurface()); 
  ...
  while  (source_y < getFrame()->getHeight() ) {
    ...
    while ( source_x < getFrame()->getWidth() ) {
      ...
      Frame* frame = 
            new Frame(VegetaSurface, chunk_width, chunk_height,
                  source_x, source_y
                );
      ChunkVegeta chunk(
